...
-- 初始数据 
INSERT INTO `sys_user` (`user_id`, `username`, `password`, `salt`, `email`, `mobile`, `status`, `create_user_id`, `create_time`) VALUES ('1', 'admin', '9ec9750e709431dad22365cabc5c625482e574c74adaebba7dd02f1129e4ce1d', 'YzcmCZNvbXocrsz9dm8e', 'cwj@chinaredsun.com', '13612345678', '1', '1', '2016-11-11 11:11:11');
...
INSERT INTO `schedule_job` (`bean_name`, `method_name`, `params`, `cron_expression`, `status`, `remark`, `create_time`) VALUES ('testTask', 'test', 'gclfax', '0 0/30 * * * ?', '0', '有参数测试', '2016-12-01 23:16:46');
...
-- ----------------------------
--  Table structure for `t_activity_base`
-- ----------------------------
DROP TABLE IF EXISTS `t_activity_base`;
CREATE TABLE `t_activity_base` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `name` varchar(50) NOT NULL COMMENT '活动名称',
  `type` int(2) NOT NULL COMMENT '活动类型1、抽奖；2、答题',
  `start_date` date DEFAULT NULL COMMENT '活动开始日期',
  `start_time` time DEFAULT NULL COMMENT '活动开始时间',
  `end_date` date DEFAULT NULL COMMENT '活动结束日期',
  `end_time` time DEFAULT NULL COMMENT '活动结束时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=101 DEFAULT CHARSET=utf8 COMMENT='活动基础信息表';
...
CREATE TABLE `t_activity_info` (
  ...
  `free_part_num` int(4) NOT NULL DEFAULT '0' COMMENT '免费参与次数\n',
  `share_add_num` int(4) NOT NULL DEFAULT '0' COMMENT '微信分享增加次数\n',
  `max_part_num` int(4) NOT NULL DEFAULT '0' COMMENT '最大参与次数\n',
  `min_xinbi` int(11) NOT NULL DEFAULT '0' COMMENT '最小奖励鑫币数量\n',
  `max_xinbi` int(11) NOT NULL DEFAULT '0' COMMENT '最大奖励鑫币数量\n',
  `min_bounty` int(11) NOT NULL DEFAULT '0' COMMENT '最小奖励奖励金数量\n',
  `max_bounty` int(11) NOT NULL DEFAULT '0' COMMENT '最大奖励奖励金数量\n',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='活动详细表';

-- ----------------------------
--  Table structure for `t_activity_question`
-- ----------------------------
DROP TABLE IF EXISTS `t_activity_question`;
CREATE TABLE `t_activity_question` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `activity_id` bigint(20) NOT NULL COMMENT '活动Id',
  `question_id` bigint(20) NOT NULL COMMENT '问题Id',
  `score` int(4) DEFAULT NULL COMMENT '分数',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='活动与题目关系表';

-- ----------------------------
--  Table structure for `t_activity_share`
-- ----------------------------
DROP TABLE IF EXISTS `t_activity_share`;
CREATE TABLE `t_activity_share` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `activity_id` bigint(20) NOT NULL COMMENT '活动Id',
  `equip_id` varchar(50) NOT NULL COMMENT '设备唯一标识（微信则是openID）',
  `user_id` bigint(20) DEFAULT NULL COMMENT '用户ID',
  `share_time` datetime NOT NULL COMMENT '分享时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='用户分享活动记录表';

-- ----------------------------
--  Table structure for `t_answer_assist`
-- ----------------------------
DROP TABLE IF EXISTS `t_answer_assist`;
CREATE TABLE `t_answer_assist` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `activity_id` bigint(20) NOT NULL COMMENT '活动Id',
  `equip_id` varchar(50) NOT NULL COMMENT '设备唯一标识（微信则是openID）',
  `award_id` bigint(20) NOT NULL COMMENT '奖励ID',
  `asist_time` datetime NOT NULL COMMENT '助力时间',
  `add_bounty` int(10) DEFAULT '0' COMMENT '增加奖励金数量',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='用户答题助力表';

-- ----------------------------
--  Table structure for `t_answer_records`
-- ----------------------------
DROP TABLE IF EXISTS `t_answer_records`;
CREATE TABLE `t_answer_records` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `activity_id` bigint(20) NOT NULL COMMENT '活动Id',
  `equip_id` varchar(50) NOT NULL COMMENT '设备唯一标识（微信则是openID）',
  `user_id` bigint(20) DEFAULT NULL COMMENT '用户ID',
  `answer_time` datetime NOT NULL COMMENT '答题时间',
  `answer_result` bit(1) DEFAULT NULL COMMENT '答题结果',
  `bounty_num` int(10) DEFAULT NULL COMMENT '获得奖励金数量',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='用户答题记录表';

-- ----------------------------
--  Table structure for `t_question_options`
-- ----------------------------
DROP TABLE IF EXISTS `t_question_options`;
CREATE TABLE `t_question_options` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `question_id` bigint(20) NOT NULL COMMENT '问题编号\n',
  `content` varchar(200) NOT NULL COMMENT '选项内容\n',
  `is_correct` bit(1) DEFAULT NULL COMMENT '是否正确',
  `order` int(2) NOT NULL COMMENT '选项排序\n',
  PRIMARY KEY (`id`)
...
CREATE TABLE `t_questions` (
  ...
  `type` int(2) NOT NULL COMMENT '题目类型1、单选题；2、多选题；3、其他',
  `status` bit(1) NOT NULL COMMENT '状态0，停用；1，启用',
  `classify` int(2) NOT NULL COMMENT '题目归类1、风险等级评估；2、平台相关；3、趣味问题',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='问题列表';

-- ----------------------------
--  Table structure for `t_user_awards`
-- ----------------------------
DROP TABLE IF EXISTS `t_user_awards`;
CREATE TABLE `t_user_awards` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `activity_id` bigint(20) NOT NULL COMMENT '活动Id',
  `equip_id` varchar(50) NOT NULL COMMENT '设备唯一标识（微信则是openID）',
  `user_id` bigint(20) DEFAULT NULL COMMENT '用户ID',
  `answer_id` bigint(20) DEFAULT NULL COMMENT '答题ID',
  `part_bounty` int(10) DEFAULT '0' COMMENT '增加奖励金数量',
  `assit_bounty` int(10) DEFAULT '0' COMMENT '增加奖励金数量',
  `is_accept` bit(1) NOT NULL DEFAULT b'0' COMMENT '是否已经领奖 1是；0否',
  `create_time` datetime NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='用户获奖记录表';
